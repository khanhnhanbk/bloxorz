...
class maps:
    ...
    def distantToTarget(self,x1,y1):
        x2,y2 = self.end
        stack = [[x2,y2]]
        visited = []
        count = 0
        ...
        while stack:
            ...
            if x == x1 and y == y1:
                return count
            next_index = [[x+1,y], [x,y+1], [x-1,y], [x,y-1]]
            for xn, yn in next_index:
                if [xn, yn] in visited:
                    continue
                if (xn < 0) or (xn >= sizex) or (yn < 0) or (yn >= sizey):
                    continue
                if (self.maps[xn][yn].type == 0):
                    continue
                stack.append([xn,yn])
            count +=1
        return 100
